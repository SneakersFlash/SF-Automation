...
- **Output on-brand (2026-07-03)**: backend `CreativeService.loadBrandContext()` inject objek `brand` (voice/audience/usp/constraints/cta/**knowledge**/**examples**) dari BrandProfile ke payload skill → brief/copy tidak generik/robotik. SKILL.md 4 agent di-upgrade (spec kaya + aturan anti-AI); pakai `brand.*` kalau ada, else baseline `brand-knowledge.md`. Content-brief & copywriting lewat pass `sf-humanize` (guarded). Field `knowledge`/`examples` diisi user via web `/settings/brand-profiles`. Edit isi SKILL.md/knowledge **tanpa restart gateway**.